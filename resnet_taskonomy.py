...
class PostEncoder(nn.Module):
    def __init__(self, block, layers,input_dim = 3,widths=[64,128,256], num_classes=1000, zero_init_residual=False,
        ...
        self.conv1 = nn.Conv2d(self.input_dim, self.inplanes, kernel_size=5, stride=2, padding=2,
                               bias=False)#64
        self.bn1 = norm_layer(self.inplanes)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)#32
        self.layer1 = self._make_layer(block, widths[0], layers[0], stride=2,
                                       dilate=replace_stride_with_dilation[0]) # 16
        ...
        self.layer3 = self._make_layer(block, widths[2], layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1]) # 4
        self.maxpool2 = nn.MaxPool2d(kernel_size = 4, stride = 1, padding = 0)
