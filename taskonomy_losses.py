...
def segment_semantic_loss(output,target,mask):
    global sl
    sl = torch.nn.functional.cross_entropy(output.float(),target.long().squeeze(dim=1),reduction='mean')
    return sl

def classification_loss(output,target):
    global clsl 
    clsl = torch.nn.functional.cross_entropy(output.float(),target.long(),reduction='mean')
    return clsl 
